...
const [showAllProjects, setShowAllProjects] = useState(false);
 ...
 const plannedTimeline = new Array(weekLabels.length).fill(0).map((_, index) => {
...
const latestStatusDate = statusHistory.length > 0 
  ? new Date(Math.max(...statusHistory.map(s => new Date(s.updated_at))))
  : null;
...
weekLabels.forEach((week, index) => {
  ...
  const weekStatus = statusHistory.find(s => {
    ...
    const weekOfStatus = getWeekFromStart(date, new Date(selectedProject.start_date));
...
const statusDifference = weekLabels.map((week, index) => {
  const statusPercent = statusTimeline[index] !== null ? statusTimeline[index] : null;
  const plannedPercent = plannedTimeline[index];
  const difference = statusPercent !== null ? statusPercent - plannedPercent : null;
      ...
      >
        ...
        <p>
          ...
          {statusHistory.length > 0 
            ? statusHistory.reduce((latest, current) => 
