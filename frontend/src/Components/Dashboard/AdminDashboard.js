...
const AdminDashboard = () => {
 ...
 const handleSubmitResponse = () => {
  ...
  {displayedProjects.map((project, index) => {
    ...
    const end = new Date(project.client_end_date);
    ...
    if (!isNaN(start) && !isNaN(end)) {
      ...
      if (years < 0) {
        ...
        const dText = days > 0 ? `${days} day${days > 1 ? "s" : ""}` : "";
        durationText = (yText + mText + dText).trim() || "0 days";
      }
    }
    ...
    return (
      <div
        key={index}
        className="col-xl-3 col-lg-4 col-md-6 col-sm-12 mb-4 d-flex align-items-stretch"
        style={{ cursor: "pointer" }}
        onClick={() => openModal(project)}
      >
        <div
          className={`card ${colorClass} text-white w-100 shadow-sm`}
          style={{
            borderRadius: "8px",
            minHeight: "220px",
            display: "flex",
        ...
        >
          <div className="card-body">
            <h5 className="card-title fw-bold">{project.project_name}</h5>
            {/* <h6 className="card-subtitle mb-2">{project.project_id}</h6>
             <strong>ID:</strong> {project.project_id}
            <p className="card-text mb-1">{displayText}</p>
            ...
            <p className="card-text mb-1">
              <strong>Status:</strong> {displayText}
            </p>
            <p className="card-text">
              <strong>Duration:</strong> {durationText}
            </p>
  ...
  {projects.length > 8 && (
    <div className="text-center mt-3">
      <button className="btn btn-outline-primary" onClick={toggleShowAllProjects}>
        {showAllProjects ? "Show Less" : "View More"}
      </button>
    </div>
  )}
